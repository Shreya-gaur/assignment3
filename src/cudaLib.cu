...
uint64_t evaluateGpuConv (TensorShape iShape, TensorShape fShape, 
	...
	// 				std::cout << out[n * oShape.width * oShape.height * oShape.channels + ch * oShape.width * oShape.height + i * oShape.width + j] << " ";
	// 			}
	// 			std::cout << "\n";
	// 		}
	// 		std::cout << "\n";
...
void convLayer_gpu ( float * input, TensorShape iShape, 
	...
	if (col_gl < oShape.width && row_gl < oShape.height) {
		...
		for (uint32_t n = 0; n < batchSize; ++ n) {
			for (uint32_t m = 0; m < oShape.channels; ++ m) {
				...
				output[n * oShape.channels * oShape.height * oShape.width + m * oShape.height * oShape.width + row_gl * oShape.width + col_gl] = conv_op;
